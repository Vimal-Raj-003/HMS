...
const dummyPrescriptions: Prescription[] = [
  {
    ...
    status: 'PENDING',
    prescriptionFileUrl: '/prescriptions/hypertension-prescription.pdf',
    notes: 'Take medications regularly. Follow up in 2 weeks.',
    medicines: [
  ...
  },
  {
    ...
    status: 'DISPENSED',
    prescriptionFileUrl: '/prescriptions/diabetes-followup.pdf',
    notes: 'Continue current treatment. Monitor blood sugar regularly.',
    medicines: [
      ...
      { name: 'Atorvastatin', dosage: '10mg', frequency: 'Once daily', duration: '30 days', instructions: 'Take at bedtime' }
    ]
  },
  {
    id: '3',
    ...
    specialization: 'General Medicine',
    diagnosis: 'Hypertension - Blood Pressure Management',
    status: 'DISPENSED',
    prescriptionFileUrl: '/prescriptions/bp-treatment.pdf',
    notes: 'Reduce salt intake. Exercise regularly.',
    medicines: [
      { name: 'Amlodipine', dosage: '10mg', frequency: 'Once daily', duration: '30 days', instructions: 'Take in the morning' },
      { name: 'Hydrochlorothiazide', dosage: '25mg', frequency: 'Once daily', duration: '30 days', instructions: 'Take with plenty of water' }
    ]
  },
  {
    id: '4',
    ...
    prescriptionFileUrl: '/prescriptions/allergy-medication.pdf',
    notes: 'Avoid known allergens. Use air purifier at home.',
    medicines: [
      { name: 'Cetirizine', dosage: '10mg', frequency: 'Once daily', duration: '14 days', instructions: 'Take at bedtime if causes drowsiness' },
      { name: 'Fluticasone Nasal Spray', dosage: '50mcg', frequency: 'Twice daily', duration: '30 days', instructions: 'Shake well before use' },
  ...
  },
  {
    ...
    status: 'PENDING',
    prescriptionFileUrl: '/prescriptions/urti-prescription.pdf',
    ...
    medicines: [
      ...
      { name: 'Paracetamol', dosage: '500mg', frequency: 'As needed', duration: '5 days', instructions: 'Take for fever/pain, max 4 times daily' },
      { name: 'Chlorpheniramine', dosage: '4mg', frequency: 'Three times daily', duration: '5 days', instructions: 'May cause drowsiness' }
    ]
  }
];
...
export default function MyPrescriptions() {
  ...
  const handleViewPrescription = (prescription: Prescription) => {
    if (!prescription.prescriptionFileUrl) {
      toast.error('Prescription file not available');
      return;
    }

    setViewingDocument({
      fileUrl: prescription.prescriptionFileUrl,
      fileName: `prescription-${prescription.doctorName.replace(/\s+/g, '-').toLowerCase()}-${new Date(prescription.date).toISOString().split('T')[0]}.pdf`,
  ...
  const handleDownloadPDF = async (prescription: Prescription) => {
    ...
    try {
      const fileUrl = prescription.prescriptionFileUrl;
      const fileName = `prescription-${prescription.doctorName.replace(/\s+/g, '-').toLowerCase()}-${new Date(prescription.date).toISOString().split('T')[0]}.pdf`;
...
---
This is a demo prescription generated for demonstration purposes.
Please present this prescription at your pharmacy to collect medicines.
        `.trim();
      ...
      try {
        const fileName = `prescription-${prescription.doctorName.replace(/\s+/g, '-').toLowerCase()}-${new Date(prescription.date).toISOString().split('T')[0]}.pdf`;
        const a = window.document.createElement('a');
        a.href = prescription.prescriptionFileUrl || '';
        a.download = fileName;
        a.target = '_blank';
        window.document.body.appendChild(a);
        a.click();
    ...
    <div className="space-y-6">
      ...
      {viewerOpen && viewingDocument && (
        <DocumentViewer
          ...
          onDownload={() => {
            const prescription = prescriptions.find(p => p.prescriptionFileUrl === viewingDocument.fileUrl);
            if (prescription) {
              handleDownloadPDF(prescription);
            }
          }}
